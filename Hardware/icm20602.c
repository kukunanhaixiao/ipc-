...
//		printf("icm_20602 reset fail\r\n");
	...
	delay_ms(50);
	resetValue = icm20602_read_reg(ICM20_PWR_MGMT_1);//读取复位后寄存器的值
	 idtype=icm20602_read_reg(ICM20_WHO_AM_I);
	//printf("icm_20602 id=%x\r\n", idtype); // 读取ID
//	OLED_ShowHexNum(1, 5,idtype, 2);
	delay_ms(50);
...
uint8_t icm20602_set_gyro_fullscale(uint8_t fs)    //角速度量程比例因子
{
	switch (fs)
	{
...
uint8_t icm20602_get_accel_adc(int16_t *accel)   //获取加速度数据
{
	uint8_t buf[6];
...
uint8_t icm20602_get_gyro_adc(int16_t *gyro)     //获取角速度数据
{
	uint8_t buf[6];
...
{  
	...
	icm20602_get_accel(icm20602_accel);
	icm20602_temp=icm20602_get_temp();
	
	if(Flag == 0)
	{
		OLED_ShowSignedNum(1, 1, icm20602_gyro[0], 2);
		OLED_ShowSignedNum(1, 5, icm20602_gyro[1], 2);
		OLED_ShowSignedNum(1, 9, icm20602_gyro[2], 2);
		OLED_ShowSignedNum(4, 14, 1, 1);
	}
	else if(Flag == 1)
	{
		OLED_ShowSignedNum(1, 1, icm20602_accel[0], 2);
		OLED_ShowSignedNum(1, 5, icm20602_accel[1], 2);
		OLED_ShowSignedNum(1, 9, icm20602_accel[2], 2);
		OLED_ShowSignedNum(4, 14, 2, 1);
	}
	else if(Flag == 2)
	{
		OLED_ShowString(1, 5, "          ");
		OLED_ShowSignedNum(1, 1, icm20602_temp, 2);
		OLED_ShowSignedNum(4, 14, 3, 1);
	}
